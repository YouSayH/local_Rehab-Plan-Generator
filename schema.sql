...
VALUES (
        ...
        '術後脱臼リスク（股関節の屈曲90度以上、内転、内旋は禁忌）。転倒による骨折リスク。',
        '医師の指示する股関節運動制限（脱臼肢位）の厳守。急な動作や無理な体勢は避ける。',
        -- 心身機能・構造
        TRUE,
        TRUE,
        TRUE,
        TRUE,
        '左股関節の術後痛あり。安静時軽度（NRS 2/10）、動作時中等度（NRS 4/10）。',
        TRUE,
        '左股関節の可動域制限あり（特に屈曲、外転）。',
        TRUE,
        '左臀筋群、大腿四頭筋の筋力低下が著明 (MMT 3レベル)。',
        TRUE,
        TRUE,
        TRUE,
        -- ADL (FIM/BI)
        ...
        7,
        7,
        '歩行は歩行器を使用。下肢の更衣、靴下の着脱に自助具（ソックスエイド、リーチャー）を使用。トイレは洋式に限定。',
        -- 栄養
        TRUE,
        168.0,
        TRUE,
        ...
        TRUE,
        -- 目標・方針・署名
        '歩行器を用いて監視下で自宅内移動が自立する。左股関節の疼痛が安静時消失し、動作時軽度となる。',
        'T字杖を用いて屋外歩行が安定して行える。ゴルフ練習場での軽いスイング動作が可能となる。自宅での家事動作（買い物、調理）が一部自立する。',
        TRUE,
        '自宅（妻と二人暮らし）',
        '人工股関節脱臼予防のための運動制限を徹底する。段階的な荷重訓練と股関節周囲筋力強化を行う。骨粗鬆症治療継続のための服薬指導と栄養指導を継続する。',
        ...
        '妻（介助負担の軽減、介護指導）',
        -- 具体的な対応方針
        'ゴルフ再開に向け、専門のプロによる指導や、体幹・下肢の安定性向上に特化したトレーニングプログラムを理学療法士と相談して作成する。',
        '退院後を見据え、自宅の環境に合わせた動作練習（椅子からの立ち上がり、浴槽への出入り）を繰り返し行う。屋外歩行では、段差や坂道など、実際の生活環境に近い場所での練習を取り入れる。',
        '住宅改修の専門家と連携し、手すりの設置場所や浴槽の改修について具体的な提案を行う。介護保険サービスの利用申請手続きをサポートする。',
        '妻に対し、股関節の脱臼予防肢位について再度説明し、日常的な介助時の注意点を指導する。'
    );
