...
"""
    ...
    def query(self, query: str):
        ...
        if self.query_enhancer:
            ...
            search_query = self.query_enhancer.enhance(query)
            print(f"  - 拡張されたクエリ (検索に使用):\n'{search_query[:150]}...'")
        else:
            print("\n[ステップ1/6] クエリ拡張(HyDE)はスキップされました。")
            search_query = query # HyDEがない場合は元のクエリをそのまま使用

        print("\n[ステップ2/6] 関連文書を検索中...")
        # 変数名を enhanced_query から search_query に変更
        results = self.retriever.retrieve(search_query, n_results=15)
