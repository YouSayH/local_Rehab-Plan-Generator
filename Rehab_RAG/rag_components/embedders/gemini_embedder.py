...
class GeminiEmbedder:
    ...
    def __init__(
        ...
        print(
            f"Embeddingモデルの初期化完了。バッチサイズ: {self.batch_size}, APIコール間の待機時間: {self.sleep_duration:.2f}秒"
        )

    @backoff.on_exception(
        backoff.expo, genai.errors.ClientError, max_tries=5, max_time=120
    )
    ...
    def embed_documents(self, texts: list[str]) -> list[list[float]]:
        ...
        for i in tqdm(range(0, len(texts), self.batch_size), desc="Embedding Batches"):
            ...
            try:
                ...
                print(
                    f"エラー: バッチ {i // self.batch_size + 1} は最大リトライ回数を超えても失敗しました。スキップします。: {e}"
                )
                ...
                print(
                    f"エラー: バッチ {i // self.batch_size + 1} で予期せぬエラーが発生しました。: {e}"
                )
                all_embeddings.extend([None] * len(batch_texts))
        ...
        if not valid_embeddings:
            raise RuntimeError(
                "全てのチャンクのエンベディングに失敗しました。APIのレート制限（特にTPM）を確認してください。"
            )

        if len(valid_embeddings) != len(texts):
            print(
                f"警告: {len(texts) - len(valid_embeddings)}個のチャンクのエンベディングに失敗しました。"
            )
