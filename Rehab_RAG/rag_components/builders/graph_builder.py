...
class KnowledgeGraph(BaseModel):
    nodes: Optional[List[Node]] = Field(
        description="抽出されたエンティティのリスト", default_factory=list
    )
    relationships: Optional[List[Relationship]] = Field(
        description="抽出された関係性のリスト", default_factory=list
    )
...
class GraphBuilder:
    def __init__(self, config: dict, db_path: str, **kwargs):
        ...
        if not all([uri, user, password]):
            raise ValueError(
                "環境変数 NEO4J_URI, NEO4J_USERNAME, NEOJ_PASSWORD が設定されていません。"
            )
...
"""
        try:
            ...
            print(
                f"警告: グラフ抽出中にエラーが発生しました。スキップします。エラー: {e}"
            )
            return None
    ...
    def _write_to_neo4j(self, graph_data: KnowledgeGraph, chunk_text: str):
        ...
        with self.driver.session() as session:
            for node in graph_data.nodes:
                ...
                session.run(
                    "MERGE (n:`{label}` {{id: $id}})".format(label=node.label.value),
                    id=node.id,
                )
