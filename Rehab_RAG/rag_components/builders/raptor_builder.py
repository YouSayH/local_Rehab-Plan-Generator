...
# 要約:"""
    ...
    def build(self):
        ...
        print(
            f"'{source_path}' からドキュメントを読み込み、レベル0のチャンク（葉）を生成中..."
        )
        base_chunks = []
        ...
        while len(current_level_texts) > 1 and level < max_levels:
            ...
            print(
                f"  - {len(unique_labels[unique_labels != -1])}個のクラスタを発見しました。(ノイズ除く)"
            )
                    ...
                    all_levels_chunks.append(
                        {
                            ...
                            "metadata": {
                                ...
                                "level": level + 1,
                                "cluster_id": f"{level + 1}-{label}",
                            },
                        }
                    )

            if not next_level_texts or len(next_level_texts) >= len(
                current_level_texts
            ):
                print(
                    f"レベル{level + 1}でチャンク数が減少しなかったため、ツリーの構築を終了します。"
                )
                break
