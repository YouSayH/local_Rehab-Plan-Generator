...
class HybridRetriever:
    ...
    def retrieve(self, query_text: str, n_results: int = 10) -> dict:
        ...
        sorted_doc_ids = sorted(
            rrf_scores.keys(), key=lambda id: rrf_scores[id], reverse=True
        )
        ...
        final_documents = [
            all_docs[doc_id]["text"] for doc_id in top_doc_ids if doc_id in all_docs
        ]
        final_metadatas = [
            all_docs[doc_id]["metadata"] for doc_id in top_doc_ids if doc_id in all_docs
        ]
