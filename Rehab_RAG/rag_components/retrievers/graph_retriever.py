...
class GraphRetriever:
    ...
    def __init__(self, llm, **kwargs):
        ...
        if not all([uri, user, password]):
            raise ValueError(
                "環境変数 NEO4J_URI, NEO4J_USERNAME, NEO4J_PASSWORD が設定されていません。"
            )
        ...
        print(
            "Graph Retriever (キーワード検索版・改) が初期化され、Neo4j に接続しました。"
        )
    ...
    def retrieve(self, query_text: str, n_results: int = 10) -> dict:
        ...
        return {
            ...
            "metadatas": [
                [{"source": "Knowledge Graph"}] * len(unique_context[:n_results])
            ],
        }
