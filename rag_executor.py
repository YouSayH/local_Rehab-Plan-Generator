...
class RAGExecutor:
    ...
    def __init__(self, pipeline_name: str):
        ...
        pipeline_config_path = os.path.join(
            "Rehab_RAG", "experiments", pipeline_name, "config.yaml"
        )
        if not os.path.exists(pipeline_config_path):
            raise FileNotFoundError(
                f"設定ファイルが見つかりません: {pipeline_config_path}"
            )
        ...
        if "common_components" in self.pipeline_config:
            ...
            specific_config = self.pipeline_config.get(
                "pipeline_specific_components", {}
            )
        else:
            print(
                "INFO: 'common_components'が見つからないため、フラットなconfigとして読み込みます。"
            )
        ...
        if "llm" in common_config:
            ...
            self.components["llm"] = get_instance(
                cfg["module"], class_name, cfg.get("params", {})
            )

        # embedderを先に初期化するロジック
        embedder_cfg = specific_config.pop(
            "embedder", None
        )  # specific_configからembedderを一旦取り出す
        if not embedder_cfg and "build_components" in self.pipeline_config:
            embedder_cfg = self.pipeline_config.get("build_components", {}).get(
                "embedder"
            )
            if embedder_cfg:
                print(
                    "INFO: 'query_components'にembedderがないため、'build_components'から読み込みます。"
                )

        if embedder_cfg:
            class_name = embedder_cfg.get("class_name") or embedder_cfg.get("class")
            self.components["embedder"] = get_instance(
                embedder_cfg["module"], class_name, embedder_cfg.get("params", {})
            )
        ...
        for name, config in specific_config.items():
            if config:
                ...
                if name == "filter" and isinstance(config, list):
                    ...
                    for filter_cfg in config:
                        ...
                        self.components["filters"].append(
                            get_instance(
                                filter_cfg["module"],
                                filter_cfg.get("class") or filter_cfg.get("class_name"),
                                params,
                            )
                        )
                ...
                params = config.get(
                    "params", {}
                ).copy()  # .copy()で元のconfigを汚染しないようにする
                ...
                for path_key in ["path", "db_path", "bm25_path"]:
                    if (
                        path_key in params
                        and isinstance(params[path_key], str)
                        and not os.path.isabs(params[path_key])
                    ):
                        # 相対パスをexperimentディレクトリからの絶対パスに変換
                        absolute_path = os.path.join(
                            self.experiment_dir, params[path_key]
                        )
                        params[path_key] = os.path.normpath(absolute_path)
                        print(
                            f"INFO: パラメータ '{path_key}' を絶対パスに変換しました: {params[path_key]}"
                        )

                if (
                    name in ["judge", "query_enhancer", "filter"]
                    and "llm" not in params
                ):
                    params["llm"] = self.components.get("llm")
                if name == "retriever":
                    ...
                    if "path" not in params and "database" in self.pipeline_config:
                        ...
                        if db_path:
                            params["path"] = os.path.normpath(
                                os.path.join(self.experiment_dir, db_path)
                            )
                            print(
                                f"INFO: retrieverの'path'をdatabaseセクションから補完: {params['path']}"
                            )

                    if (
                        "collection_name" not in params
                        and "database" in self.pipeline_config
                    ):
                        params["collection_name"] = self.pipeline_config.get(
                            "database", {}
                        ).get("collection_name")
                ...
                self.components[name] = get_instance(
                    config["module"], class_name, params
                )
        ...
        self.filters = self.components.get(
            "filters", []
        )  # 'filters' (複数形) を取得。なければ空リスト
...
"""
    ...
    def execute(self, patient_facts: dict):
        print(
            f"DEBUG [rag_executor.py]: '担当者からの所見' received = {patient_facts.get('担当者からの所見')}"
        )
        ...
        query_for_retrieval = json.dumps(
            patient_facts, ensure_ascii=False, indent=2, default=str
        )
        ...
        if self.filters and docs:  # self.filter を self.filters に変更
            ...
            print(
                f"フィルタリング後、{len(docs)}件の文書が残りました。 ({original_doc_count - len(docs)}件を除外)"
            )
        else:
            print("フィルタリングはしません。")
        ...
        return {"answer": answer_dict, "contexts": final_contexts_with_metadata}
